...
use App\Services\Inventory\GenerateStockBalanceReportService;
...
class GenerateInventoryReportTest extends TestCase
{
    ...
    public function test_it_generates_stock_balance_report_file_and_export_record(): void
    {
        [$product, $unit, $location] = $this->prepareMasterData();
        $movement = $this->createMovement($product, $unit, $location, null, 'confirmed', '10.0000');
        $this->createMovement($product, $unit, $location, null, 'confirmed', '-3.0000');

        $export = app(GenerateStockBalanceReportService::class)
            ->generate(reason: 'stock balance report');

        $absolutePath = storage_path('app/'.$export->file_path);
        $content = file_get_contents($absolutePath);

        $this->assertSame('stock_balance', $export->report_type);
        ...
        $this->assertDatabaseHas('report_exports', [
            ...
            'reason' => 'stock balance report',
        ]);

        $this->assertDatabaseHas('audit_logs', [
            'event' => 'stock_balance_report.generated',
    ...
    public function test_it_keeps_reissued_stock_balance_reports_as_separate_files(): void
    {
        [$product, $unit, $location] = $this->prepareMasterData();
        $this->createMovement($product, $unit, $location, null, 'confirmed', '10.0000');

        $first = app(GenerateStockBalanceReportService::class)
            ->generate(reason: 'first stock balance report');
        $second = app(GenerateStockBalanceReportService::class)
            ->generate(reason: 'second stock balance report');
    ...
    public function test_it_generates_lot_stock_balance_report_file_and_export_record(): void
    {
        ...
        $movement = $this->createMovement($product, $unit, $location, $lot, 'confirmed', '10.0000');
    ...
    public function test_it_rejects_empty_stock_balance_report_generation(): void
    {
        $this->expectException(InventoryReportExportException::class);

        app(GenerateStockBalanceReportService::class)->generate();
    }
    ...
    public function test_it_rejects_unsupported_report_format(): void
    {
        ...
        app(GenerateStockBalanceReportService::class)->generate('pdf');
    }
    ...
    private function prepareMasterData(): array
    {
        ...
        $product = Product::create([
            'product_code' => 'INVENTORY-REPORT-SAKE-001',
            'product_type' => 'sake',
            'name' => 'Inventory Report Sake',
            'display_name' => 'Inventory Report Sake 720ml',
