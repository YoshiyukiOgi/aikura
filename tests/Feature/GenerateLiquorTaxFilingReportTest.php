...
class GenerateLiquorTaxFilingReportTest extends TestCase
{
    ...
    public function test_it_generates_xlsx_and_pdf_confirmation_sheets(): void
    {
        ...
        $this->assertSame(['e-Tax転記確認表', '転記チェック', '根拠明細', '軽減計算根拠', '不足情報'], $book->getSheetNames());
        $this->assertSame('酒税納税申告 e-Tax転記確認表', $book->getSheet(0)->getCell('A1')->getValue());
        $this->assertSame('清酒', $book->getSheet(0)->getCell('A18')->getValue());
        $this->assertSame('課税移出', $book->getSheet(0)->getCell('B18')->getValue());
        $this->assertSame('15度', $book->getSheet(0)->getCell('C18')->getValue());
        $this->assertSame('課税標準数量', $book->getSheetByName('転記チェック')->getCell('B2')->getValue());
        $this->assertSame(2160, $book->getSheetByName('転記チェック')->getCell('D2')->getValue());
        $this->assertSame('申告者情報', $book->getSheetByName('不足情報')->getCell('A2')->getValue());
        $this->assertSame('未登録', $book->getSheetByName('不足情報')->getCell('B2')->getValue());
        $this->assertSame('清酒', $book->getSheetByName('根拠明細')->getCell('D2')->getValue());
        $this->assertSame('課税移出', $book->getSheetByName('根拠明細')->getCell('E2')->getValue());
        $this->assertSame('15度', $book->getSheetByName('根拠明細')->getCell('F2')->getValue());
        $this->assertSame('清酒', $book->getSheetByName('軽減計算根拠')->getCell('C2')->getValue());
        $this->assertSame('課税移出', $book->getSheetByName('軽減計算根拠')->getCell('D2')->getValue());
        $this->assertSame('15度', $book->getSheetByName('軽減計算根拠')->getCell('E2')->getValue());
        ...
        $this->assertSame('legacy_quantity_limit', $book->getSheetByName('軽減計算根拠')->getCell('I2')->getValue());
