...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}[hidden]{display:none!important}body{margin:0}h1,h2,h3,p{margin:0}
    ...
    main{padding:12px 18px 40px}.layout{display:grid;grid-template-columns:360px minmax(0,1fr);gap:12px;align-items:start}.card{min-width:0;border:1px solid #dce4ee;border-radius:6px;background:#fff;overflow:hidden}.card.list-card{max-height:calc(100vh - 76px);overflow:auto}.card.detail-card{position:sticky;top:64px;max-height:calc(100vh - 76px);overflow:auto;align-self:start}.head{display:flex;align-items:center;justify-content:space-between;gap:10px;min-height:46px;padding:9px 12px;border-bottom:1px solid #e7edf4}.head h2{font-size:14px}.actions,.filter-actions{display:flex;align-items:center;gap:7px}.muted{color:#64748b;font-size:11px;line-height:1.5}
    button,input,select,textarea{font:inherit;color:#172033}button{min-height:30px;padding:5px 9px;border:1px solid #cad6e6;border-radius:4px;background:#fff;cursor:pointer}button:hover{border-color:#8ebcff}button:disabled{opacity:.45;cursor:not-allowed}.primary{background:#0b6ff6;color:#fff;border-color:#0b6ff6;font-weight:800}@keyframes searchPulse{0%,100%{background:#fff7d6;border-color:#f0b429;box-shadow:0 0 0 0 rgba(240,180,41,.28)}50%{background:#ffe08a;border-color:#d89b00;box-shadow:0 0 0 5px rgba(240,180,41,.12)}}button.search-attention,button.primary.search-attention{animation:searchPulse 1.8s ease-in-out infinite;color:#172033!important;font-weight:800}.secondary{color:#075ecf;border-color:#a9caff}.danger{color:#b42318;border-color:#f0b6b1}
    ...
    @media(max-width:1120px){.layout{grid-template-columns:1fr}.list-card{max-height:480px}.detail-card{position:static;max-height:none}.form-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
...
<body>
  ...
  <main>
    <x-basic-master-tabs active="customers" />
    <div class="layout">
  ...
  <script>
    ...
    el('filters').onsubmit=event=>{event.preventDefault();state.page=1;loadList()};el('filters').addEventListener('input',()=>setFilterDirty(true));el('filters').addEventListener('change',()=>setFilterDirty(true));el('reset-filter').onclick=()=>{el('filters').reset();state.page=1;loadList()};el('prev-page').onclick=()=>{if(state.page>1){state.page--;loadList()}};el('next-page').onclick=()=>{if(state.page<state.lastPage){state.page++;loadList()}};el('reload-customer').onclick=()=>state.selectedId&&loadCustomer(state.selectedId);el('new-customer')?.addEventListener('click',newCustomer);el('customer-form').onsubmit=saveCustomer;document.querySelectorAll('.tab').forEach(tab=>tab.onclick=()=>activateTab(tab.dataset.tab));el('customer-form').addEventListener('input',()=>{state.dirty=true});el('customer-form').addEventListener('change',()=>{state.dirty=true});window.addEventListener('beforeunload',event=>{if(state.dirty){event.preventDefault();event.returnValue=''}});loadList();
  </script>
</body>
</html>
