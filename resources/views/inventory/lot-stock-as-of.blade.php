...
<main>
  ...
  <section class="card">
    ...
    <div class="toolbar">
      ...
      <label>ロット検索<input id="keyword" type="search" value="{{ $keyword ?? '' }}" placeholder="ロット名・ロットコード"></label>
    ...
    <div class="table-wrap">
      <table>
        ...
        <tbody id="rows">
          <tr><td colspan="7" class="empty">読み込み中です。</td></tr>
        </tbody>
      </table>
    </div>
  </section>
...
(() => {
  ...
  const setSearchDirty = (dirty = true) => els.search?.classList.toggle('search-attention', dirty);
  ...
  async function load() {
    ...
    els.rows.innerHTML = '<tr><td colspan="7" class="empty">読み込み中です。</td></tr>';
    try {
      ...
      if (!rows.length) {
        els.rows.innerHTML = '<tr><td colspan="7" class="empty">該当するロット在庫はありません。</td></tr>';
        return;
      }
      els.rows.innerHTML = rows.map((row) => `
        <tr class="${Number(row.physical_quantity) < 0 ? 'negative-stock' : ''}">
          <td>${esc(row.product_code)}</td>
          <td class="lot-cell"><span class="lot-name">${esc(row.lot_name || row.lot_code)}</span>${row.lot_name && row.lot_name !== row.lot_code ? `<span class="sub">コード: ${esc(row.lot_code)}</span>` : ''}</td>
      ...
      els.rows.innerHTML = `<tr><td colspan="7" class="empty">${esc(error.message)}</td></tr>`;
      els.count.textContent = '';
    }
  }

  els.search.addEventListener('click', load);
  els.keyword.addEventListener('input', () => setSearchDirty());
  els.date.addEventListener('change', () => setSearchDirty());
  if (els.showZero) els.showZero.addEventListener('change', () => setSearchDirty());
  ...
  if (els.date) els.date.addEventListener('change', (event) => { event.stopImmediatePropagation(); setSearchDirty(); }, true);
  if (els.showZero) els.showZero.addEventListener('change', (event) => { event.stopImmediatePropagation(); setSearchDirty(); }, true);
  if (els.clear) els.clear.addEventListener('click', (event) => { event.stopImmediatePropagation(); els.date.value = '{{ $today }}'; els.keyword.value = ''; if (els.productType) els.productType.value = 'sake'; load(); }, true);
  if (els.productType) els.productType.addEventListener('change', () => setSearchDirty());
